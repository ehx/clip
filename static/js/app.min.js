...
function mainFunction($scope, notificationResource, $timeout, $http, $location, loginService, localStorageService, notificationService, $rootScope, $auth) {
    // if user have token
...
function todoFunction($scope, todoResource, $timeout, $routeParams, loginService, $route) {
    ...
    $scope.minDate = function () {
        if ($scope.dates.minDate) {
            return $scope.dates.minDate
        } else {
            return time.now();
        }
    }
...
function SignUpController($auth, $location, $scope) {
    $scope.signup = function () {
        ...
        $auth.signup(user).then(function () {
            $auth.login(user_login).then(function () {
                $location.path("/");
            })
        }).catch(function (response) {
            Materialize.toast('Ocurrio un error al generar el nuevo' +
                ' usuario.', 2000, '');
        });
    }
}
...
function LoginController($auth, $location, $scope) {
    $scope.login = function () {
        ...
        $auth.login(user).then(function (data) {
            ...
            Materialize.toast('Los datos ingresados son incorrectos.', 2000, '');
        });
    }
}