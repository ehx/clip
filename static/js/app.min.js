 ...
 $location, toaster, fileUpload, loginService, urgencyResource, statusResource, moduleResource, userService, configurationResource, $q) {
  ...
  $scope.deleteComment = function(commentId) {
    if (confirm("Esta seguro de borrar el comentario ?")) {
      ...
      toaster.pop('success', 'Comentario', 'El comentario se borro correctamente.');
      $timeout(getComments, 300);
      $timeout(getAttachments, 300);
    }
  };
  ...
  $scope.makeFavourite = function(comment) {
    ...
    taskCommentsResource.update({ id: comment.id }, { 
      ...
      toaster.pop('success', 'Comentario', 'El usuario marco/desmarco como destacado el comentario.');
    }, function(){
      toaster.pop('error', 'Comentario', 'El usuario no tiene permiso para marcar como destacado.');
    });
    $timeout(getComments, 300);
  };
  ...
  $scope.doneTask = function() {
    if (confirm("Esta seguro de cerrar la tarea ?")) {
      ...
      taskResource.update({
        ...
        toaster.pop('error', 'Tarea', 'El usuario no tiene permiso para cerrar la tarea.');
      });
    }
  };
  ...
  $scope.openTask = function() {
    if (confirm("Esta seguro de abrir la tarea ?")) {
      ...
      taskResource.update({
        ...
        toaster.pop('error', 'Tarea', 'El usuario no tiene permiso para abrir la tarea.');
      });
    }
  };
  ...
  $scope.updateTask = function (){
    taskResource.update({
        ...
        toaster.pop('sucess', 'Tarea', 'Se actualizo la tarea.');
        $timeout(getTask, 300) 
      }, function(err){
        toaster.pop('error', 'Tarea', 'El usuario no tiene permiso para modificar esta tarea');
      }
    );
  }
...
function tipFunction($scope, tipResource, loginService, $timeout) {
  ...
  $scope.filterCategory = function(tip) {
    var catIsNew = indexedCategory.indexOf(tip.category) == -1;
    if (catIsNew) {
        indexedCategory.push(tip.category);
    }
    return catIsNew;
  }
