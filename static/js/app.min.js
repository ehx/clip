 ...
 $location, toaster, fileUpload, loginService, urgencyResource, statusResource, moduleResource, userService, configurationResource, $q) {
  ...
  $scope.deleteComment = function(commentId) {
    if (confirm("Esta seguro de borrar el comentario ?")) {
      ...
      toaster.pop('success', '', 'El comentario se borro correctamente.');
      $timeout(getComments, 300);
      $timeout(getAttachments, 300);
    }
  };
  ...
  $scope.makeFavourite = function(comment) {
    ...
    taskCommentsResource.update({ id: comment.id }, { 
      ...
      toaster.pop('success', '', 'El usuario marco/desmarco como destacado el comentario.');
    }, function(){
      toaster.pop('error', '', 'El usuario no tiene permiso para marcar como destacado.');
    });
    $timeout(getComments, 300);
  };
  ...
  $scope.deleteTask = function() {
    if (confirm("Esta seguro de borrar la tarea ?")) {
      ...
      cTask.$delete();
      $location.path("/");
      toaster.pop('success', '', 'La tarea se borro correctamente.');
    }
  };
  ...
  $scope.doneTask = function() {
    if (confirm("Esta seguro de cerrar la tarea ?")) {
      ...
      taskResource.update({
        ...
        toaster.pop('success', '', 'La tarea se cerro correctamente.');
      }, function(){
        toaster.pop('error', '', 'El usuario no tiene permiso para cerrar la tarea.');
      });
    }
  };
  ...
  $scope.openTask = function() {
    if (confirm("Esta seguro de abrir la tarea ?")) {
      ...
      taskResource.update({
        ...
        toaster.pop('error', '', 'El usuario no tiene permiso para abrir la tarea.');
      });
    }
  };
  ...
  $scope.updateTask = function (){
    taskResource.update({
        ...
        toaster.pop('sucess', '', 'Se actualizo la tarea.');
        $timeout(getTask, 300) 
      }, function(err){
        toaster.pop('error', '', 'El usuario no tiene permiso para modificar esta tarea');
      }
    );
  }
  ...
  $scope.updateComment = function (activeComment){
    if(activeComment.comment){
      ...
      toaster.pop('error', '', 'El comentario no puede estar vacio');
    }
  }
}
...
function tipFunction($scope, tipResource, loginService, $timeout, toaster) {
  ...
  $scope.updateTip = function(tip){
    tipResource.update({ id: tip.id }, tip, function(){
      toaster.pop('success', '', 'El tip fue actualizado.');
      this.close();
    }, function(){
      toaster.pop('error', '', 'Ocurrio un error, el tip no fue actualizado.');
    });
  }
}
